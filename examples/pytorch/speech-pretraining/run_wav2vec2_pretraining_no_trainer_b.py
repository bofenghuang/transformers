...
class DataCollatorForWav2Vec2Pretraining:
    ...
    max_length: Union[float, int] = 30 * 16_000

    def __call__(self, features: List[Dict[str, Union[List[int], torch.Tensor]]]) -> Dict[str, torch.Tensor]:

        tmp_features = []
        # features is list of examples
        for example in features:

            waveform, _ = get_waveform(
                example[self.audio_column_name],
                frames=example["n_frames"],
                start=example["start_in_frames"],
            ...
            # print(waveform.shape[0] / self.model_sampling_rate)

            inputs = self.feature_extractor(
                waveform, sampling_rate=self.model_sampling_rate, max_length=self.max_length, truncation=True
            )
            tmp_features.append({"input_values": inputs.input_values[0]})
        ...
        # print(mask_time_indices.sum(axis=1) - 1)
...
def main():
    ...
    datasets_splits = [
        load_dataset("csv", data_files="/projects/bhuang/corpus/speech/weak_hmhm_37k/data_oct.tsv", sep="\t")["train"],
        load_dataset("csv", data_files="/projects/bhuang/corpus/speech/weak_hmhm_37k/data_dec_9_14.tsv", sep="\t")["train"],
        load_dataset("csv", data_files="/projects/bhuang/corpus/speech/weak_hmhm_37k/data_dec_14_25.tsv", sep="\t")["train"],
    ]
    ...
    def prepare_dataset(batch):
        ...
        start = int(float(batch["start"]) * sampling_rate)
        end = int(float(batch["end"]) * sampling_rate)

        batch["start_in_frames"] = start
        batch["n_frames"] = end - start
    ...
    with accelerator.main_process_first():
        vectorized_datasets = raw_datasets.map(
            ...
            remove_columns=["start", "end", "ID", "wav_name"],
            cache_file_names=cache_file_names,
            load_from_cache_file=not args.overwrite_cache,
        )

        # logger.info(f'Raw training data duration: {sum(vectorized_datasets["train"]["n_frames"]) / sampling_rate / 3600:.2f}h')
        # logger.info(f'Raw validaiton data duration: {sum(vectorized_datasets["validation"]["n_frames"]) / sampling_rate / 3600:.2f}h')
        ...
        if min_length > 0.0:
            vectorized_datasets = vectorized_datasets.filter(
                ...
                input_columns=["n_frames"],
            )
            logger.info(f"Filtered dataset: {vectorized_datasets}")

            # logger.info(f'Filtered training data duration: {sum(vectorized_datasets["train"]["n_frames"]) / sampling_rate / 3600:.2f}h')
            # logger.info(f'Filtered validaiton data duration: {sum(vectorized_datasets["validation"]["n_frames"]) / sampling_rate / 3600:.2f}h')
