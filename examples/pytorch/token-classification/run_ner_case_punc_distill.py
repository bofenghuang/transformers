...
from utils.disillation_utils import DistillationTrainer, DistillationTrainingArguments
...
# def main(args):
    ...
    def init_student_model():
        return RobertaForCasePunc.from_pretrained(
            model_args.model_name_or_path,
            num_case_labels=len(label_lists_by_choice["case"]),
            num_punc_labels=len(label_lists_by_choice["punc"]),
            from_tf=bool(".ckpt" in model_args.model_name_or_path),
    ...
    def compute_objective(metrics: Dict[str, float]) -> float:
        ...
        selected_metrics = {k: v for k, v in metrics.items() if "punc_" in k and "_f1" in k}
        return sum(selected_metrics.values())
