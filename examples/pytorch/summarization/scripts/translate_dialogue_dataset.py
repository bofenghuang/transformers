...
def translate_dialogue(s):
    try:
        dialogue_turns = s.split("\n")

        new_dialogue_turns_list = []
        for dialogue_turn in dialogue_turns:
            # speaker, sentence = dialogue_turn.split(": ", maxsplit=1)
            speaker, sentence = dialogue_turn.split(":", maxsplit=1)
...
def translate_example(example):
    example["dialogue_fr"] = translate_dialogue(example["dialogue"])
    example["summary_fr"] = translate_sentences(example["summary"])
